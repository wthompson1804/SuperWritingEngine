...
function inline(c) {
  let s = String(c);
  s = s.replace(/<(strong|b)\b[^>]*>([\s\S]*?)<\/\1>/gi, (m, t, x) => { const v = stripTags(x).trim(); return v ? `**${v}**` : ''; });
  s = s.replace(/<(em|i)\b[^>]*>([\s\S]*?)<\/\1>/gi, (m, t, x) => { const v = stripTags(x).trim(); return v ? `*${v}*` : ''; });
  ...
  return decode(stripTags(s)).replace(/[ \t]*\n[ \t]*/g, ' ').replace(/\s{2,}/g, ' ').trim();
}
